function ProductCard({category, products}) {
    ...
    return ( 
        ...
        <p className="max-w-2xl mx-auto mt-4 text-[#915858]/80 text-sm leading-relaxed">
          Discover our selection of {category} — each piece carefully crafted with
          elegance and precision for timeless beauty.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 px-8 pb-20 max-w-6xl mx-auto">
        {products.map((item) => (
          <div
            key={item._id}
            className="text-center bg-[#FBE8E8] hover:bg-[#F9D9D9] transition rounded-xl p-6"
          >
            <img
              src={item.image}
              alt={item.name}
